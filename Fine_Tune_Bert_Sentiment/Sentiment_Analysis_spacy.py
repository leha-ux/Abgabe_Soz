...
def custom_sentiment_analysis(df, text_column):
    """
    Applies sentiment analysis using the custom spaCy model.
    ...
    Returns:
        pandas.DataFrame: DataFrame with sentiment labels and scores.
    """
    # Predict sentiment using the custom spaCy model
    sentiments = df[text_column].apply(nlp_custom)

    # Assuming your spaCy model outputs the sentiment as 'POSITIVE' or 'NEGATIVE' in a 'sentiment' attribute,
    # and a sentiment score in a 'sentiment_score' attribute
    df['Sentiment_Label'] = [doc._.sentiment for doc in sentiments]
    df['Sentiment_Score'] = [doc._.sentiment_score for doc in sentiments]
...
modell_processed = custom_sentiment_analysis(df_tokenize, 'satz')
...
nlp_custom = spacy.load('spacy_model')
